...
**输出语言（最高优先级，必须严格遵守）**：先判断【待提取的新消息】的主导语言——只要消息中包含中文，所有自由文本字段（\`scene_name\`、memory \`content\`）就**必须全部使用简体中文输出**，禁止输出英文；消息全英文时才使用英文。JSON 字段名、枚举值、ISO 时间戳保持英文。
...
【角色扮演场景（重要）】此对话是 AI 扮演虚构角色与真实用户进行角色扮演。消息归属：role=user 的消息作者是真实用户（提取主体一律写"用户（真实用户的称呼/名字）"）；role=assistant 的消息作者是 AI 扮演的角色。提取时用户消息中自称的"我/俺/人家"等一律指真实用户，绝不可把用户的信息记为 AI 的。
...
**输出语言（最高优先级，必须严格遵守）**：先判断待提取消息的主导语言——只要消息中包含中文，所有自由文本字段（\`scene_name\`、memory \`content\`）就**必须全部使用简体中文输出**，禁止输出英文；消息全英文时才使用英文。JSON 字段名、枚举值、ISO 时间戳保持英文。
