...
- 📋 演进路线图（剩余优化方向按优先级排序）：[ROADMAP-2026-08.md](ROADMAP-2026-08.md)
...
12. **grok2api 上游会话失效**（外部依赖，2026-08-07 排查确认）：高频调用后 grok.com 对账号会话限流/失效（chat 502 upstream_network_error、quota 刷新失败、本地账号状态仍 active、网络/容器重建均无法解决）；恢复靠新账号（注册机补号）或会话冷却（可能数小时-24h）；排查要点：面板 API（/api/admin/v1/accounts）查账号、refresh-quota 验证会话、egress 节点勿乱禁用。**备用网关方案（已启用）**：memory-core 的 LLM 可切 cpa（cli-proxy-api:8317，gpt-oss-120b-medium）——.env 设 TDAI_LLM_BASE_URL/TDAI_LLM_API_KEY/TDAI_LLM_MODEL 即可；平台文本/对话本身会自动降级到 cpa（DB ProviderConfig 多 provider 容错）
