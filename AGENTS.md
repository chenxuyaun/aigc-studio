...
🎯 GPU 节点接入(2026-08-25 ✅ 音乐+视频全链路打通)：用户家 RTX 4080SUPER 16G 麒麟 V10(172.168.10.160, root 密码 SSH, 已授权接管)。frps --network host(0.0.0.0:7000 + 服务口绑 172.17.0.1); token 在 .build-tmp/_frp_token.txt; 配置 /home/ubuntu/frps/frps.toml。GPU 端 /root/gpu-node/ docker-compose：comfyui(7865 自建镜像)+musicgen(8010)+frpc，隧道已通。hub 注册 provider **必须传 provider_type 字段**(传 type 被忽略成 openai_compatible)。坑八连：① Windows CRLF 拆坏命令→sed 's/\\r$//' ② 麒麟机只有 docker-compose(无 compose 插件) ③ frps 容器内 172.17.0.1 是容器自己的网关→必须 host 网络 ④ comfyanonymous/comfyui 不在 daocloud 白名单→自建 Dockerfile ⑤ ComfyUI torch>=2.4(custom_op)/MusicGen torch>=2.6(CVE-2025-32434 torch.load weights_only 限制 .bin 权重；safetensors 不受限)→基镜像 2.4.1/2.6.0 ⑥ GitHub 直连/ghproxy 全挂→云服务器经 Clash 认证代理 clone 打包传入 ⑦ frpc localPort 用容器内端口 ⑧ 麒麟机 8000 已有 Myolotrain，宿主端口避开。 完成：music 槽=MusicGen(torch2.6)→wav 落库(c7c31c88)；video 槽=ComfyUI0.33 Wan2.1→mp4 落库(338985a5, ftyp 323KB)。ComfyUI0.33 附加坑：EmptyLatentVideo 已移除→官方模板 EmptyHunyuanLatentVideo+ModelSamplingSD3+CreateVideo+SaveVideo(format=auto, VIDEO 类型输入)；模型挂载 /app/ComfyUI/models(WORKDIR 相对路径)；comfy_kitchen 0.2.31 的 list[int] 注解与 torch2.6 infer_schema 不兼容→ck_patch.py 改 typing.List；triton JIT 需 gcc；poll 须阻塞轮询(首次无记录 continue, running 也 continue)。批13 终验修复(2026-08-26)：🔴 task_runner use_real 门禁在 hub 链解析之前——video 无环境默认模型名直接拒绝，hub video 槽永远走不到；修复=空 model 先问 hub 槽位链首有真实候选即放行。test_gpu_node 与阻塞轮询实现不兼容（旧单次 poll 断言致 pytest 挂起 12min）改两段式+超时用例。公网 E2E 实证：music 114s wav 落库、video 144s mp4 347KB ftyp 落库、浏览器同源拉取 200。⚠️ access-url 的 sig 短时效，脚本复制 URL 延迟会假 401/502，验收须浏览器同源即时拉取。GPU 节点离线时 frps 正常但隧道端口不监听，属用户家物理层，代码无需处置。 批14(2026-08-26, 4e29a21)：① MiniMax·Hailuo(H3) 视频 provider 上线(app/providers/minimax_video.py, type=minimax_video)入 video 链备选=[ComfyUI主选→H3云API]，GPU 离线自动降级；API 姿势=POST /v2/video_generation(Bearer, model=MiniMax-H3, content[], resolution/duration/ratio 必填) → GET /v1/query/video_generation?task_id=(Preparing/Processing/Success+file_id) → files/retrieve 换 download_url；**api_key 留空待用户填**(模型中心供应商抽屉或给 key 即通)。② 复制按钮无效根因=navigator.clipboard 仅安全上下文(http 公网直访为 undefined)，13 处散落直调统一收敛 lib/clipboard.ts copyText()(opacity 0.01+iOS setSelectionRange+返回 boolean 不抛未处理异常)；公网浏览器 E2E：点📋→✓已复制反馈、零 JS 报错。 批15 收尾(2026-08-26, 8cd6ba5)：MiniMax-H3 本地跑通验证失败——如实结论=**4080S 16G 物理跑不动 H3**(TE qwen3vl32B 必须整体进内存/显存，DM fp8 20G 加载再叠内存→两次 OOM，286s 后抛 Prompt executed 失败)。坑与发现：① 用户 160 的 /data/ai-video/comfyui(宿主 8188 ComfyUI，venv_conda 21G 常驻)模型库**早有整套 H3**(8-06 下载，int8 扩散 21G+TE nvfp4+双 VAE)——容器 /root/gpu-node/comfy_models 是另一套，查模型必看 /data；② 内存 62G 被 8188(21G)+容器(21G)占满→H3 加载需 ~36G 峰值，腾内存=临时停 8188(kill -INT pid；恢复 cmd=/data/ai-video/comfyui/venv_conda/bin/python main.py --listen 0.0.0.0 --port 8188)，**venv 解释器是项目内 venv_conda 非 miniconda env**(误用 /nwh/miniconda3/envs/vllm35 起不来报 ModuleNotFoundError sqlalchemy)；③ ComfyUI 前沿 LoraLoader 必填 clip，H3 模板已补(8cd6ba5)；④ frpc 隧道 session shutdown 自动重连但频繁抖动→云端查 7001 不稳,直查 160 本地 127.0.0.1:7860 最可靠。video 链已收敛=[本地GPU·ComfyUI(Wan2.1)] 唯一主选；用户确认**无 MiniMax API Key**→云H3 与本地H3 均不可用，已从链摘除(provider 定义保留，有 key 或换 24G+ 卡可恢复)。H3 线到此闭环。 ⚠️ video/music 槽=外部资源硬约束（2GB 无 GPU 跑不动 Wan2.1 等本地模型，Suno/免费音乐 API 均需注册付费），如实保持槽空+UI 诚实报拒绝，不硬上。

## 交易 Agent + Qwen 本地接入 + 媒体链根治（2026-09-01）

**✅ ten 交易 Agent 模型钉扎**：`ten/backend/app/services/ai_trader/llm_client.py` request_llm 增 `AI_TRADER_MODEL` env 过滤 hub 文本链；ten-api systemd drop-in `proxy.conf` 设 `AI_TRADER_MODEL=gpt-oss-120b-medium`——防止 trader 自动跟随链首 claude 烧共享 antigravity 配额。cpa 日志实证只走 gpt-oss。

**✅ 160 节点 Qwen3 接入模型中心**：ollama 0.23.2 起不动 qwen3next 架构（layer 64 missing attn_qkv）→ 升 0.33.2（安装器坑：装 zstd 后仍下载截断→**Windows SFTP 中转 tarball** 到 160 解 /usr/local）；qwen3.8-27b-ud-iq2_s 8.4G 全 GPU offload ~1s 短答，`/no_think` 关思考；hub text 链尾=160·Qwen3(172.17.0.1:7004/v1) 作 antigravity 全挂时的兜底（实测降级生效）。ollama 常驻与媒体生成撞显存：已加 `/etc/systemd/system/ollama.service.d/keep-alive.conf` → `OLLAMA_KEEP_ALIVE=1m`（收窄碰撞窗口）；媒体生成前可 `POST /api/generate {"model":…,"keep_alive":0}` 立即卸载（/v1 忽略 keep_alive，必须走 /api/generate）。

**🔴✅ 媒体链三重根因连环（2026-09-01 video/music 连败复盘，提交 443ce06/95fd8ff/cd619f2/5ef8934）**：
1. **`orchestrator._resolve_model_name` 是同步函数却调协程** `get_active_chain`（无 await）→ coroutine 对象 truthy 但 `chain[0]` 抛 TypeError 被 except 吞 → 空 model 的 video 恒报「未配置 Provider」。修复=函数改 async + orchestrator/task_runner 两调用点 await。**教训：先 A/B stash 对照再判断是不是自己的回归；协程误用要在 CI 就炸出来。**
2. **audio/music 分支缺 conf=None 防护**：hub 瞬态不可达时 `_media_candidates` 退化 `[None]`，image/video 分支有 None 防护、audio/music 没有 → `conf[2]` 炸 'NoneType' not subscriptable。补齐 + `MODEL_HUB_TIMEOUT` 默认 2s→5s + 空链 1.5s 重试一次 + 退化/异常显式日志（media_candidate_exception 带 traceback）。
3. **真正的雷：`candidate.py` 用 stdlib logging 却写 structlog 风格 kwargs**（`logger.info("media_skip_offline_gpu", provider_type=…)`）→ 走到「本地 GPU last_ok!=1 跳过」分支必抛 `Logger._log() unexpected keyword`，被外层 except 吞掉 → **候选链静默崩成 [None]**。今日 frpc 隧道断过一次，hub 探活把 ComfyUI 记 last_ok=0 → 跳过分支被触发 → 雷爆。改位置参数格式。**教训：任何 `except: pass` 包着的多候选链路，退化路径必须有日志；接入点先查 logger 是 stdlib 还是 structlog。**

**✅ 五路真实链 E2E 全绿（2026-09-01 生产实证）**：text=claude-sonnet-4-6(hub)、image=**FLUX 本地**(comfyui_image candidate 1/3——修复前那次其实走 registry→cpa 兜底，修复后才真走本地)、video=**ComfyUI Wan2.1 本地**(candidate 1/2)、audio=Edge-TTS、music=MusicGen 本地。媒体候选验证法：worker 日志 `media_candidate_used candidate=N/M provider_type=…`。

**🔴 160 容器 Exited(137) 未自动拉起 + frpc.toml 被还原为模板**（serverAddr/token 全错）：compose 有 `restart: unless-stopped` 却 5 小时没人拉 → 高度怀疑 160 上 **gitlab-runner CI** 用 docker stop/down 干预（与 frpc.toml 还原同源嫌疑）。处置：`docker-compose up -d comfyui musicgen` 拉起 + sed 修 frpc.toml 真实 token。**经验：云端 7001/7002=000 而 7004=200 时，不是隧道问题是 160 容器停了**（frpc 一个进程管全部隧道）；诊断顺序 = 云端 curl 隧道 → 160 `docker ps` → 160 本地 curl 容器端口。
