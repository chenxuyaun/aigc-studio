...
import { Feather, Loader2, PenLine, Plus, Sparkles, Trash2, X } from "lucide-react";
...
export function VoiceBadge({ className }: { className?: string }) {
  ...
  const [profile, setProfile] = useState<VoiceProfileDto | null>(null);
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await apiClient.get<{ exists: boolean; profile?: VoiceProfileDto }>(
        "/voice/profile",
      );
      setExists(!!r.exists);
  ...
  return (
    <div ref={boxRef} className={cn("relative", className)}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1.5 w-80 max-w-[92vw] rounded-2xl border border-border bg-surface-raised p-4 shadow-2xl backdrop-blur-xl">
            ...
            <div className="space-y-3 text-xs">
              ...
              <label className="block">
                <span className="mb-1 flex items-center gap-1 text-[10px] uppercase tracking-wide text-muted-foreground">
                  <Plus className="h-3 w-3" aria-hidden /> 粘贴新样本（会更新文风）
                </span>
                <textarea
                  value={sample}
