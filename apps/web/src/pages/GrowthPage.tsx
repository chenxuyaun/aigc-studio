...
export default function GrowthPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-6">
      <header>
        <h1 className="text-xl font-semibold text-foreground">AI 成长足迹</h1>
      ...
      <section className="ai-glass rounded-2xl p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-medium text-foreground">
            AI 记得的你
            <span className="ml-2 text-[11px] text-muted-foreground">{memories.length} 条</span>
          </h2>
        </div>
        {memories.length === 0 ? (
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {memories.map((m) => {
              ...
              return (
                <li
                  key={m.id}
                  className="group flex max-w-full items-center gap-1.5 rounded-full border border-line bg-muted/40 px-3 py-1.5 text-xs text-muted-foreground"
                >
                  <span className="font-medium">{meta.label}</span>
                  <span className="max-w-[16rem] truncate text-foreground">{m.content}</span>
                  <button
                    type="button"
                    onClick={() => void removeMemory(m.id)}
                    className="ml-0.5 text-muted-foreground opacity-0 transition-opacity hover:text-rose-300 group-hover:opacity-100"
                    title="让 AI 忘记这条"
                  >
                    ✕
                  </button>
        ...
        <ol className="mt-4 flex flex-col gap-4 border-l border-white/10 pl-4">
          {diaries.map((d) => (
            <li key={d.id} className="relative">
              <span
                className="absolute -left-[21px] top-1.5 h-2.5 w-2.5 rounded-full bg-primary/40"
                aria-hidden
              />
