...
import { estimateTokens, type ChatMsg, type ChatSession, type CharacterItem, type Persona, type QuickReply } from "@/pages/roleplay/types";

/** 快捷回复的轻量宏展开（{{char}}/{{user}}/{{random::a::b}}/{{time}}）。 */
function expandQuickMacros(text: string, charName: string, userName: string): string {
  let out = text
    ...
    .replace(/<CHAR>/g, charName)
    .replace(/<USER>/g, userName);
  out = out.replace(/\{\{random::([^}]+)\}\}/g, (_m, opts: string) => {
