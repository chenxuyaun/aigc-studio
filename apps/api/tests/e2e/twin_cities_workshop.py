...
def main() -> None:
    ...
    # 2. 项目级世界书：世界观
    for kw, content in [
        (["双城"], "临海双城：A 城（西岸，旧港商埠，老城街巷密集）与 B 城（东岸，新兴医疗城，全城三成人口从事医疗行业），隔江相望，轮渡二十分钟。两城警方分属不同分局，平时鲜有交集。"),
        (["交换杀人手抄本"], "残破的线装手抄本，封面无字，内页以钢笔抄录「交换杀人之法则」：甲乙互不相识，互杀对方之目标，动机湮灭，无人可证。纸页有茶渍与煤灰痕迹，落款日期被撕去。该手抄本是本案最大误导源——它的出现本身就是一个精心设计的饵。"),
        (["旧伤"], "两具尸体的尸检中，法医发现同一处异常：两人都曾接受过同一侧锁骨旧骨折手术，手术方式高度一致——这指向一座早已废弃的 Z 城矿工医院，也是连接两位死者旧事的唯一实体证据。"),
    ]:
    ...
    if "error" in r:
        ...
        check("群聊对抗审问（四角色博弈）", True, f"{r.get('turns')} 轮对话 · {r.get('word_count')} 字")
