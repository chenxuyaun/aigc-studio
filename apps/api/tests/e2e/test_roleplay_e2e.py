...
def main():
    ...
    check(
        "character import",
        imported.get("ok") is True and bool(imported.get("asset_id")),
        r.text[:120],
    )
    ...
    check(
        "stream mood",
        done is not None and done.get("mood", "") != "",
        str(done.get("mood", "")) if done else "",
    )
    ...
    r = c.post(
        ...
        headers=h,
        files={"file": ("c.jsonl", jsonl.encode(), "application/x-ndjson")},
    )
    ...
    r = c.post(
        ...
        headers=h,
        json={
            "character_name": detail.get("name"),
            "keywords": ["月亮石"],
    ...
    check(
        "lore updated fields",
        target
        and target["constant"] is True
        and target["position"] == "after"
        and "月石" in target["keywords"],
        str(target)[:120] if target else "missing",
    )
    check("lore delete", c.delete(f"/roleplay/lore/{lid}", headers=h).json().get("ok") is True)

    # 8. regex 脚本 + 应用到回复
    r = c.post(
        "/roleplay/regex-scripts",
        headers=h,
        json={
            "name": "清理星号",
            "pattern": r"\*",
    ...
    check(
        "chat non-stream 200",
        r.status_code == 200 and bool(resp.get("reply")),
        resp.get("error", ""),
    )
    check(
        "regex applied (no asterisks)", "*" not in resp.get("reply", ""), resp.get("reply", "")[:80]
    )
    check(
        "regex delete",
        c.delete(f"/roleplay/regex-scripts/{rid}", headers=h).json().get("ok") is True,
    )
    ...
    r = c.post(
        "/roleplay/personas",
        headers=h,
        json={"name": "艾拉", "description": "一位来自深海的探险家。"},
    )
    pid = r.json().get("id")
    ...
    check(
        "persona delete", c.delete(f"/roleplay/personas/{pid}", headers=h).json().get("ok") is True
    )
    ...
    check(
        "quick reply delete",
        c.delete(f"/roleplay/quick-replies/{qid}", headers=h).json().get("ok") is True,
    )
