...
def main() -> None:
    ...
    r = c.post(
        f"/story/chapters/{cid1}/generate",
        headers=h,
        json={
            "project_id": pid,
            "mode": "narrative",
    ...
    gen = r.json()
    check(
        "narrative chapter generated",
        "error" not in gen and gen.get("status") == "done",
        r.text[:120],
    )
    ...
    r = c.post(
        ...
        headers=h,
        json={"title": "山顶夜话", "outline": "两位角色在山顶相遇"},
    )
    cid2 = r.json()["chapter"]["id"]
    r = c.post(
        f"/story/chapters/{cid2}/generate",
        headers=h,
        json={
            "project_id": pid,
            "mode": "script",
    ...
    check(
        ...
        "error" not in script and script.get("turns", 0) >= 1,
        r.text[:120],
    )
    ...
    # 6. 创作团队：主编 → 剧务
    r = c.post(
        f"/story/projects/{pid}/crew", headers=h, json={"project_id": pid, "stage": "director"}
    )
    check("crew director", r.status_code == 200 and bool(r.json().get("direction")), r.text[:100])
    r = c.post(
        f"/story/projects/{pid}/crew", headers=h, json={"project_id": pid, "stage": "stagehand"}
    )
    ...
    with c.stream(
        "POST",
        f"/story/chapters/{cid3}/generate/stream",
        headers=h,
    ...
    r = c.post(
        f"/story/projects/{pid}/schedules",
        headers=h,
        json={
            "interval_minutes": 30,
            "batch_size": 1,
    ...
    check(
        "lore default excludes project", all(i.get("project_id") != pid for i in r.json()["items"])
    )
    ...
    r = c.post(
        f"/story/chapters/{cid4}/generate/task",
        headers=h,
        json={
            "project_id": pid,
            "mode": "narrative",
