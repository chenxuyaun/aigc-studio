...
def _validate_lyrics(lyrics: str) -> list[str]:
    ...
    # 空洞赞颂词检测：赞颂句式填空词（步伐/鼓点/星火/路标/光芒/梦想/辉煌/灯塔/力量）
    # 出现 ≥3 处说明在对着模糊对象喊口号——人物没立住、句子没落地
    _HOLLOW_WORDS = ("步伐", "鼓点", "星火", "路标", "光芒", "梦想", "辉煌", "灯塔", "力量", "时代")
    hollow_hits = [w for w in _HOLLOW_WORDS if w in text]
    if len(hollow_hits) >= 3:
        warnings.append(
            f"空洞赞颂词过密（{''.join(hollow_hits)}），"
            "疑似对着模糊对象喊口号：请给'你'立人物（身份+一件具体的事），"
            "并把这些抽象词换成具体动作与物件"
        )
