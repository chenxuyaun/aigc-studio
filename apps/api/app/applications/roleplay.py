...
from app.applications import sessions
from app.applications.character_card import parse_character_card
from app.applications.macros import substitute as substitute_macros
...
from app.applications.worldbook import match_worldbook
...
def _record_memory_turn(
    ...
    if not user_id or not asset_id or not chat_id:
        return
    from app.applications.memory_client import memory_add_conversation
...
async def _build_prompt(
    ...
    *,
    ...
    if not group or len(cards) <= 1:
        try:
            from app.applications.memory_inject import build_memory_injection
...
def _chat_memory_summary(chat: Any) -> str:
    """读取会话已存摘要。"""
    from app.applications import sessions as _s
    ...
    *,
    ...
    if session_id and work_messages and not swipe:
        ...
        if mem_chat is not None:
            ...
            from app.applications.status_book import (
                apply_book,
                book_to_text,
                merge_settings,
                parse_annotations,
    ...
    from app.applications.worldbook import estimate_tokens as _est_tok
