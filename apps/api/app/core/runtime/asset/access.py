"""Core Runtime - Asset 访问控制（URL 签名 / 验证 / 签发）。

从 services/media_access.py 抽离：
- MediaAccess: Pydantic model
- sign_content_url: HMAC 签名（24h 短期 URL）
- verify_content_signature: 常量时间比较
- issue_media_access: 按 backend 选 signed/local 路径
...
from app.core.runtime.storage import get_storage
