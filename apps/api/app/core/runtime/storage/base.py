"""Core Runtime - 对象存储抽象（BaseStorage ABC）。

从 app/storage/base.py 抽离（保留原 ObjectStorage 类名向后兼容，
同时引入 BaseStorage 别名以匹配新命名）。

P0 边界：所有 Storage Provider 必须继承此 ABC。
"""
...
# 新别名：BaseStorage 更准确表达"通用存储抽象"，P 阶段清理时统一命名
