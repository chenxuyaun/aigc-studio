"""定时创作（Scheduled Creations）核心服务。

- next_run_at 计算：daily → 下一个 HH:MM（Asia/Shanghai，固定 UTC+8 无夏令时）；
  interval → now + interval_hours。
- enqueue_run：把一条调度的 prompt 变为 GenerationTask 并入队。
  text 走同步 provider.generate（现有 run_media_task 不支持 text 类型，text.py 也是同步路径）；
  image/audio 复用 generation_service.create_media_task（同 generations 路由的创建函数）。
- run_due_schedules：扫描 is_enabled=1 且 next_run_at<=now 的调度，逐条入队并推进时间，
  异常写 last_error。被 celery beat tick（tasks/schedule_tasks.py）每分钟调用。
"""
